...
public class OntologySet 
{
	...
	// getters and setters
	public void addNumericalValue(String ontologyKeyName, String attributeStrValue, Integer numericalValue)
	{
		/* check ontologyList to see if ontology already exists for given attributeName
		 * if not create new ontology and add to set
		 */
		Ontology o = this.ontologySet.get(ontologyKeyName);
		
		if (o == null) 
			// create new ontology and add to set
			o = createNewOntologyInSet(ontologyKeyName);
		
		if (this.logSwitch) System.out.println("Adding ontology map for :"+ontologyKeyName+" : "+attributeStrValue+" : "+numericalValue);
		o.addValueMap(attributeStrValue, numericalValue);	
	}
	
	public Integer getNumericalValue(String ontologyKeyName, String attributeStrValue)
	{
		/* get numericalValue mapped to attributeStrValue in ontology keyed by ontologyKeyName
		 * 
		 */
		...
		if (o == null) {
			...
			// create new ontology and add to set
			o = createNewOntologyInSet(ontologyKeyName);
			o.addValueMap(attributeStrValue, Const.MIN_SIMILARITY_WEIGHT);
		}

		return o.getValueMap(attributeStrValue); 
    		...
    		while((line = br.readLine()) != null) {
    			...
    			if (line.length()>0) 
    			{
    				...
    				} else
    				{
    					...
    					// attribute or ontology key name | attribute value | numerical value
    					// e.g. (shape, circle, 2)
    					// e.g. (fill-h,bottom-left,3)
    					if (contents.length == 3)
    					{
    	    				this.addNumericalValue(contents[0], contents[1], Integer.parseInt(contents[2]));
    ...
    public ArrayList<NameValuePair> getFrameDataSet(TextRavensAttribute ra)
    {
    	...
    	for (NameValuePair keyMapPair : this.keyMap)
    	{
    ...
    {	
    	 ...
    	 */
    		...
    		if (useOntology) 
    		{
    			valueOfSlotFiller = this.getNumericalValue(slotName, slotFiller);
    		} else
    		{
